	...
	EasySocial.Controller("Photos.Popup", {
		...
		init: function() {
			...
			$(document).on('keyup.photos.popup', body, function(event) {
				...
				var targetIsComment = $(event.target).is('[data-comments-form-input]');
		...
		"{photo} click": function(photo, event) {
			...
			// Quick fix
			if (target.filter("[data-es-photo]").length < 1 && targetTree.filter("[data-photo-menu], [data-cover-menu]").length > 0) {
				return;
			}
