...
EasySocial.Controller('Workflows', {
	...
	save: function() {
		...
		// Simply check if tabs is not hidden, means there are still mandatory fields left
		if (!self.fieldsTypeTab().filter('[id="core"]').hasClass(opts.hiddenClass)) {
	...
	getAvailableFields: function() {
		return self.fetchWorkflowData(false, true);
	},
	...
	hideFieldBrowser: function() {
		self.fieldBrowser().addClass(opts.hiddenClass);
	},

	showDialogError: function() {
		self.dialogError().removeClass(opts.hiddenClass);
	},
	...
	addStep: function() {
		...
		self.deleteStep().removeClass(opts.hiddenClass);
	...
	addField: function(element, field, wrapper) {
		...
		if (fieldsContent.children().length > 0) {
			...
			fieldsContent.append(self.getNewFieldButton().attr('data-action', 'after'));
	...
	"{fieldBrowserClose} click": function() {
		self.hideFieldBrowser();
	},
	...
	"{deleteStep} click": function(el, event) {
		...
		confirmDelete.on('click', function(event) {
			...
			// Get the step
			var step = $(self.step.selector + '[data-id=' + id + ']');
			...
			if (self.steps().children(self.step.selector).length < 2) {
				self.deleteStep().addClass(opts.hiddenClass);
			}

			// Delete all fields within the step
			delete opts.stepsValue[id];
