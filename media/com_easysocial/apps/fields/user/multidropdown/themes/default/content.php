	...
	<ul class="g-list-unstyled" data-field-multidropdown-list>
		...
		<?php if ($selected) { ?>
			<?php foreach ($selected as $value) { ?>
				<?php echo $class->loadTemplate('input', array('inputName' => $inputName, 'choices' => $choices, 'value' => $value)); ?>
			<?php } ?>
		<?php }  ?>

		<?php if (!$count) { ?>
			<?php echo $class->loadTemplate('input', array('inputName' => $inputName, 'choices' => $choices, 'value' => ''));?>
		<?php } ?>
	</ul>
</div>
