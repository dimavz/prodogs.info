...
class PlgUserEasySocial extends EasySocialPlugins
{
	...
	public function onUserAfterSave($user, $isnew, $success, $msg)
	{
		...
		if (!$isnew) {
			...
			// is the caller
			$activations = $this->input->get('activations', '', 'default');

			if (($options == 'com_users' || $options == 'com_payplans') && ($tasks == 'activate' || $tasks == 'activate_user')) {
				$isActivation = true;
			} else {
				return true;
			}
		...
		if ($state) {
			...
			if ($isActivation && $table->state == SOCIAL_USER_STATE_PENDING) {

				$table->state = SOCIAL_USER_STATE_ENABLED;
				$table->store();

				// to support payplans user activation. we need to sync the user if
				// the activation is coming from payplan.
				if ($options == 'com_payplans' && ($tasks == 'activate' || $tasks == 'activate_user')) {
